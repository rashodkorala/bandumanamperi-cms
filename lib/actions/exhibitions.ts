...
export async function getExhibitions(): Promise<Exhibition[]> {
  ...
  data?.forEach((artworkDB) => {
    ...
    exhibitions.forEach((exhibition) => {
      const key = `${exhibition.name}|${exhibition.location}|${exhibition.date}`
...
export async function addExhibitionToArtworks(
  ...
  if (!exhibition.name.trim() || !exhibition.location.trim() || !exhibition.date) {
    throw new Error("Exhibition name, location, and date are required")
  }
...
export async function updateExhibition(
  ...
  if (!newExhibition.name.trim() || !newExhibition.location.trim() || !newExhibition.date) {
    throw new Error("Exhibition name, location, and date are required")
  }
  ...
  for (const artworkDB of data || []) {
    ...
    const updatedHistory = currentHistory.map((e) => {
      if (
        ...
        e.location === oldExhibition.location &&
        e.date === oldExhibition.date
      ) {
